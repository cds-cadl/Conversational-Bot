...
if 'last_chat' not in st.session_state:
    st.session_state.last_chat = None  # store the last query and its output
...
# Create tabs: "Chat" and "History"
tabs = st.tabs(["Chat", "History"])

with tabs[0]:
    st.subheader("Chat with Todd")
    ...
    with st.form("chat_form", clear_on_submit=True):
        ...
        if submitted and user_query:
            with st.spinner("Processing your query..."):
                ...
                # Prepare display content for the current chat
                chat_content = ""
                ...
                if "error" in parsed_output:
                    ...
                    # Display relevant action sequences
                ...
                # Save this chat in session state and logs
    ...
    # If there's a last chat, display it until a new prompt is submitted
...
with tabs[1]:
    ...
    if st.session_state.logs:
        # Convert session logs to a DataFrame for display
        df_logs = pd.DataFrame(st.session_state.logs, columns=csv_header)
        st.dataframe(df_logs, use_container_width=True)
    else:
        st.info("No chat history yet.")

# Section to download the session-specific log as CSV (available in any tab)
