...
session_keys = [
    ...
    'system_query_template', 'inference_mode', 'prev_inference_mode'
]
for key in session_keys:
    if key not in st.session_state:
        ...
        elif key == 'prev_inference_mode':
            st.session_state[key] = "Knowledge Base"

# ------------------------ Inference Mode Selection and History Reset ------------------------
...
# Check if mode has changed; if so, reset conversation history and inform the user.
...
def save_logs_to_git(selected_user, log_df):
    try:
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            # Create a folder "KB_DIRECT" then a folder for the selected user within it.
    ...
    if st.session_state.generated_responses:
        ...
        if confirm_button and selected_option:
            ...
            if st.session_state.log_data:
                ...
                # Do not clear log_data so users can download logs later
