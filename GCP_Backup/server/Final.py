...
def initialize_csv_file(path):
    try:
        with open(path, 'w', newline='', encoding='utf-8') as file:
            ...
            writer.writerow(['index', 'prompt', 'history', 'responses', 'chosen_response', 'emotion', 'personalize'])
    except Exception as e:
        logging.error(f"Failed to create CSV: {e}")

def append_to_csv_file(path, full_history):
    try:
        with open(path, 'a', newline='', encoding='utf-8') as file:
            writer = csv.writer(file)
            # Write the conversation history data
            for index, (partner_prompt, model_responses, user_response, history_snapshot, emotion, personalize) in enumerate(full_history, start=1):
                responses_list = model_responses.get('responses', [])
                history_str = ";\n".join(["\n".join(map(str, pair)) for pair in (history_snapshot or [])])
                responses_str = ';\n'.join(map(str, responses_list))
                writer.writerow([str(index), str(partner_prompt), history_str, responses_str, str(user_response), str(emotion), str(personalize)])
    except Exception as e:
            ...
            try:
                ...
                if prefix == 'prompt':
                    ...
                    if prompt:
                        ...
                        # Append to CSV file
                        append_to_csv_file(csv_file_path, [(prompt, response, None, conversation_history[-3:], emotion, personalize)])
                    else:
                    ...
                    if chosen_response:
                        ...
                        if conversation_history and conversation_history[-1][1] is None:
                            ...
                            append_to_csv_file(csv_file_path, [conversation_history[-1]])
                        else:
